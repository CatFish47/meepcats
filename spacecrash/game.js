...
function update() {
  ...
  offsetX = players[id].x - $canvas.width / 2;
  offsetY = players[id].y - $canvas.height / 2;
...
function drawShips() {
  for (var i in players) {
    if (players[i]) {
      context.beginPath();
      context.save();
      context.translate(players[i].x - offsetX, players[i].y - offsetY);
      ...
      if (players[i].dead) {
        ...
        players[i].image.src = "enemyShip.png";
      }
...
function drawObstacles() {
  for (var i = 0; i < o.length; i++) {
    context.beginPath();
    context.drawImage(o[i].image, o[i].x - o[i].size / 2 - offsetX,
      o[i].y - o[i].size / 2 - offsetY, o[i].size, o[i].size);
    context.closePath();
  }
}
...
function drawBoard() {
  ...
  context.drawImage(spaceImg, -offsetX, -offsetY, mapWidth, mapHeight);
  context.closePath();
}
...
function drawShooting() {
  if (!players[id].dead) {
    ...
    context.moveTo(players[id].x - offsetX, players[id].y - offsetY);
